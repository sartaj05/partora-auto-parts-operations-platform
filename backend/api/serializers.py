...
def quotation_dict(q):
    return {
        ...
        "items": [{"id": item.id, "sku": item.product.sku, "product": item.product.name, "quantity": item.quantity, "unit_price": float(item.unit_price), "line_total": float(item.line_total)} for item in q.items.select_related("product").all()],
        "status": q.status,
